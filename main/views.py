...
def add_to_cart(request):
    cart_p = {
        ...
        'qty': request.GET.get('qty', '1'),  # Default to '1' if 'qty' is not provided
    ...
    if 'cartdata' in request.session:
        if cart_p['id'] in request.session['cartdata']:
            cart_data = request.session['cartdata']
            cart_data[cart_p['id']]['qty'] = cart_p['qty']
            cart_data.update(cart_data)
            request.session['cartdata'] = cart_data
        else:
            cart_data = request.session['cartdata']
            cart_data.update({cart_p['id']: cart_p})
            request.session['cartdata'] = cart_data
    else:
        request.session['cartdata'] = {cart_p['id']: cart_p}
